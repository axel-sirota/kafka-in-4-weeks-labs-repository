...
public class Producer {
	...
	public static void main(String[] args) throws InterruptedException  {
		...
		for (int i = 0; i < 300; i++) {
			...
			String[] disasters = {"flood", "hurricane", "earthquake", "tornado"};
			int idx = new Random().nextInt(disasters.length);
			String key = (disasters[idx]);
			ProducerRecord<String, Double> producerRecord = new ProducerRecord<>(args[0], key, value);

			log.info("Sending message with key " + key + " and value" + value + " to topic:" + args[0]);
